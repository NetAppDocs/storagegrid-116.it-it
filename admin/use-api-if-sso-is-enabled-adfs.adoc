...
[role="lead"]
Se lo hai fatto xref:../admin/configuring-sso.adoc[SSO (Single Sign-on) configurato e abilitato] Se si utilizza Active Directory come provider SSO, è necessario emettere una serie di richieste API per ottenere un token di autenticazione valido per l'API Grid Management o l'API Tenant Management.
