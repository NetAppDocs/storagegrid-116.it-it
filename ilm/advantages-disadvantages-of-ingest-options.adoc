...
NOTE: Prima di selezionare un livello di coerenza, leggere la descrizione completa dei controlli di coerenza nelle istruzioni per xref:../s3/consistency-controls.adoc[S3] oppure xref:../swift/storagegrid-swift-rest-api-operations.adoc[Rapido] applicazioni client. Prima di modificare il valore predefinito, è necessario comprendere i vantaggi e le limitazioni.
