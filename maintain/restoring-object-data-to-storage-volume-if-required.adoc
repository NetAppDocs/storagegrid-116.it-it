...
---
= Ripristinare i dati dell'oggetto nel volume di storage, se necessario
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Cancellazione dei dati codificati (EC)
--
Se la griglia contiene dati con codifica per la cancellazione, utilizzare `start-ec-volume-repair` con il `--nodes` opzione per identificare il nodo. Quindi, aggiungere il `--volumes` oppure `--volume-range` come illustrato negli esempi seguenti.
