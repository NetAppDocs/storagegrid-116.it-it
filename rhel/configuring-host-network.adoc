...
.Di cosa hai bisogno
* Hai esaminato il xref:../network/index.adoc[Linee guida per il networking StorageGRID].
* Sono state esaminate le informazioni su xref:node-container-migration-requirements.adoc[requisiti per la migrazione dei container di nodi].
...
xref:creating-node-configuration-files.adoc[Creazione di file di configurazione del nodo]
...
Per attivare la clonazione MAC, consultare xref:creating-node-configuration-files.adoc[istruzioni per la creazione dei file di configurazione del nodo].
...
.Informazioni correlate
xref:example-etc-sysconfig-network-scripts.adoc[Esempio di /etc/sysconfig/network-scripts]
