...
.Di cosa hai bisogno
xref:placing-appliance-into-maintenance-mode.adoc[Portare il controller E5700SG in modalità di manutenzione].
...
+
Se sono state apportate modifiche alle impostazioni della VLAN, la subnet dell'appliance potrebbe essere cambiata. Se è necessario modificare gli indirizzi IP dell'appliance, seguire la procedura xref:setting-ip-configuration-sg5700.adoc[Impostare la configurazione IP] istruzioni.
