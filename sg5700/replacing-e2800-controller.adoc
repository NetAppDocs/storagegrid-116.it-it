...
* Se tutti i dischi sono protetti, è stata esaminata la procedura descritta nella procedura di sostituzione del controller simplex serie E2800, Che includono il download e l'installazione di Gestione storage e-Series SANtricity dal sito di supporto NetApp e l'utilizzo della finestra di gestione aziendale per sbloccare le unità protette dopo la sostituzione del controller.
+
...
Il nodo di storage dell'appliance non sarà accessibile quando si sostituisce il controller. Se il controller della serie E2800 funziona a sufficienza, è possibile xref:placing-appliance-into-maintenance-mode.adoc[Portare il controller E5700SG in modalità di manutenzione].
...
.Fasi
. Se l'appliance StorageGRID è in esecuzione in un sistema StorageGRID, xref:placing-appliance-into-maintenance-mode.adoc[Portare il controller E5700SG in modalità di manutenzione].
