...
.Al termine
Dopo aver riavviato l'appliance e aver verificato che il sistema KMS è stato cancellato e che l'appliance si trova in uno stato di preinstallazione, è possibile rimuoverlo fisicamente dal sistema StorageGRID. Vedere xref:../maintain/preparing-appliance-for-reinstallation-platform-replacement-only.adoc[istruzioni per la preparazione dell'apparecchio per la reinstallazione].
