...
NOTE: Se il programma di installazione dell'appliance StorageGRID non è disponibile o la pagina di accesso non viene visualizzata, è necessario utilizzare xref:setting-ip-addresses-for-storage-controllers-using-storagegrid-appliance-installer.adoc[Indirizzi IP per i controller di storage]. Accedere a Gestore di sistema SANtricity accedendo all'IP del controller di storage.
...
+
Per istruzioni specifiche sulla configurazione di un proxy StorageGRID per l'invio di messaggi AutoSupport e-Series senza utilizzare la porta di gestione, consultare xref:../admin/configuring-storage-proxy-settings.adoc[istruzioni per la configurazione delle impostazioni dello storage proxy].
