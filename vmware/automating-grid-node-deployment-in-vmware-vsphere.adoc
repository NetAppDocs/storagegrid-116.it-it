...
* Sono state raccolte tutte le informazioni necessarie per il file di configurazione. Vedere xref:collecting-information-about-your-deployment-environment.adoc[Raccogliere informazioni sull'ambiente di implementazione] per informazioni.
...
* *ADMIN_IP*: L'indirizzo IP del nodo Admin primario sulla rete Grid. Utilizzare il valore specificato come GRID_NETWORK_IP per il nodo di amministrazione primario. Se si omette questo parametro, il nodo tenta di rilevare l'IP del nodo di amministrazione primario utilizzando mDNS. Per ulteriori informazioni, vedere xref:how-grid-nodes-discover-primary-admin-node.adoc[In che modo i nodi della griglia rilevano il nodo di amministrazione primario].
+
...
xref:navigating-to-grid-manager.adoc[Accedere a Grid Manager]
